  ...
  methods: {
    ...
    loginByPassword() {
      ...
      this.$refs.loginFormRef.validate(async (valid) => {
        ...
        if (!valid) return;
        //登录权限有效时间
        var expire = this.autoLogin ? 7 * 24 * 60 * 60 : 24 * 60 * 60;
        //发起登录请求
        const { data: loginRes } = await this.$axios.post("user/login", {
        ...
        this.$message({
          ...
          center: true,
          type: loginRes.code === 200 ? "success" : "error",
          duration: 1500,
          showClose: true,
        });
      });
    ...
    regist() {
      ...
      this.$refs.registFormRef.validate(async (valid) => {
        ...
        if (!valid) return;
        //发起请求
        const { data: registRes } = await this.$axios.post("user/register", {
